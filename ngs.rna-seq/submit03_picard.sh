...
for file in `find $DIROUT -type f -name '*.bam' | sort`; do
        ...
        #picard.jar MarkDuplicates INPUT=$DIROUT"/"$sample"_sorted-coord.bam" OUTPUT=$DIROUT"/"$sample"_sorted-coord_markdups.bam" METRICS_FILE=$DIROUT"/"$sample"_sorted-coord_markdups.txt" REMOVE_DUPLICATES=true CREATE_INDEX=true
		#echo $file $sample
		#break
		#sleep 1
done
