...
submit1() {
	qsub -b y -pe threaded 10-24 -l h_vmem=50G -V -j y -m e -M dozmorovm@omrf.org -cwd -q all.q -N bt2_$3 "bowtie2 -p \$NSLOTS --local --very-sensitive-local --no-unal --mm -x /Volumes/hts_core/Shared/igenomes/Homo_sapiens/UCSC/hg19/Sequence/Bowtie2Index/genome -1 $1 -2 $2 | samtools view -bS - | samtools sort - 03_bowtie-sickle/$3.sorted && samtools index 03_bowtie-sickle/$3.sorted.bam" 
}
...
k=0
for file in `find 02_sickle -type f -name *_merged.fq.gz | sort`; do
	if [ $k = 0 ]
	then
		f1=`basename $file`
		f2=$file
