...
submit_script() {
	qsub -b y -l h_vmem=8G -V -j y -m e -M dozmorovm@omrf.org -cwd -q all.q -N fastqc_$2 "fastqc -t \$NSLOTS -o 01_fastqc-tmatic --noextract $1" 
}
...
#k=0
for file in `find 01_tmatic -type f -name '*_pair.fq.gz'`; do
#	if [ $k = 0 ]
#	then
